...
MixedGC: G1回收器独有，只回收价值高的，垃圾最多的Region区
...
6 在方法区，方法区又在永久代，full回收

7 方法区在堆内存，所以在堆内存，full回收

8 方法区在元空间，字符串常量在堆空间老年代
...
[bottom, **previous TAMS**]  建立bitMap存放**存活的对象**

[previous TAMS, **next TAMS**] 存放灰色对象，并发标记访问

- **并发标记结束后**会preTams = nextTams**表示灰色扫描完了**，进入下个扫描区域，**存活区域扩大**

[next TAMS, **top**]   并发标记阶段，新对象分配的位置
